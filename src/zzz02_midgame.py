...
class Midgame():
    ...
    class GridSquare():
        ...
        def __init__(self, p_hori = 0, p_vert = 0, p_grid = None, \
            ...
            self.vert = p_vert
            self.grid = p_grid
            self.rect = Rect(self.get_x(), self.get_y(), self.grid.grid_size, self.grid.grid_size)
    ...
    def establish_grid_neighbors(self, p_width, p_height):
        ...
        for x in range(0, p_width//self.grid_size):
            for y in range(0, p_height//self.grid_size):
                neighbors = [None]*4
                neighbors[0] = self.grid[x-1][y] if x!=0 else None   # up neighbor
                neighbors[1] = self.grid[x][y-1] if y!=0 else None    # left neighbor
                neighbors[2] = self.grid[x][y+1] if y!= p_height//self.grid_size-1 else None   # Right Neighbor
                neighbors[3] = self.grid[x+1][y] if x!= p_width//self.grid_size-1 else None   # Bottom Neighbor
    ...
    def zoom(self, grid_size_modifier):
        ...
        for x in range(len(self.grid)):
            ...
            for y in range(len(self.grid[x])):
                ...
                self.grid[x][y].rect = Rect(x_pos, y_pos, self.grid_size, self.grid_size)
